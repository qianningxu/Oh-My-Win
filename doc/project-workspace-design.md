...
| Workspace tabs bar | Bottom bar containing the Project menu and workspace tabs | Centered content-sized WinDock glass surface with 80% color-neutral backdrop blur, a 75% white tint, and a fully opaque theme-aware border plus an unclipped native shadow following the rounded surface, floating 12 points above the usable screen edge; active or hovered workspace labels are fully opaque and medium weight, while inactive labels are 80% opaque and regular weight, without an underline treatment |
| Workspace frame | Window layout area | No enclosing fill; each window and window tab bar owns its surface |
| Window tab bar | Compact WinDock glass track with a color-neutral backdrop, 25% blur opacity, a visible 25% white tint, and an independently rendered 45% theme-aware border. Focused and hovered labels are fully opaque, unfocused labels are 80% opaque, and icons remain fully opaque; focused labels use semibold weight, unfocused labels use regular weight, the focused segment uses an 80% white fill, and hover uses a 50% white fill. |
